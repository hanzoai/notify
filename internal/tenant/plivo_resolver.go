...
//
// IAM lives at hanzo.id / lux.id / zoo.id / pars.id / hanzo.id; each
...
//
//   2. Hanzo-default path. The brand has not wired its own Plivo.
//      KMS still holds `kms.hanzo.ai → brand/hanzo/plivo/*` — that's
//      Hanzo's fleet-shared account. ResolvePlivoConfig returns
//      those.
//
// Fail-closed: if the default Hanzo record is missing or unreachable,
...
// the Hanzo creds for someone else's transient KMS outage would
// risk sending the wrong brand's SMS during the outage window.
//
// On step 2 the resolver fail-closes: a missing or unreachable
// Hanzo default returns an error. notify callers surface 503.
//
...
func (r *PlivoResolver) ResolvePlivoConfig(ctx context.Context, brand string) (*PlivoConfig, error) {
	...
	if brand != DefaultBrand {
		...
		// Fall through to the Hanzo default.
	}

	// Step 2 — Hanzo default. Errors here are fatal.
