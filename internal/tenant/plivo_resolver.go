...
//
//   2. Default-brand path. The brand has not wired its own Plivo. KMS
//      still holds `kms.hanzo.ai → brand/hanzo/plivo/*` — that's the
//      fleet-shared Hanzo account. ResolvePlivoConfig returns those.
//
// Fail-closed: if the default-brand record is missing or unreachable,
...
type PlivoConfig struct {
	...
	// resolver fell back to the default brand, Brand == DefaultBrand.
	...
	// own KMS entries (not the default brand). Used by the platform
...
//  2. brand/hanzo/plivo/*       — the default brand.
//
...
// the default creds for someone else's transient KMS outage would
// risk sending the wrong brand's SMS during the outage window.
//
// On step 2 the resolver fail-closes: a missing or unreachable
// default-brand record returns an error. notify callers surface 503.
//
