...
type Config struct {
	...
	// fleet because notify. proxies inbound to all envs.
	Env string
	...
	// alternative header. Defaults to "notify." when empty.
	MailtoDomain string

	// HTTPSHost overrides the computed notify.{env}. host.
...
// notify. — no double dot, no /main/ noise.
