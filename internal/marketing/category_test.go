...
func TestAppendHTMLFooter(t *testing.T) {
	t.Parallel()
	got := AppendHTMLFooter("<p>hi</p>", "https://notify.dev./v1/notify/unsubscribe/TOK")
	if !strings.Contains(got, "<p>hi</p>") {
		t.Fatalf("original body lost: %s", got)
	}
	if !strings.Contains(got, `<a href="https://notify.dev./v1/notify/unsubscribe/TOK">click here</a>`) {
...
func TestAppendPlainFooter(t *testing.T) {
	t.Parallel()
	got := AppendPlainFooter("hello", "https://notify./v1/notify/unsubscribe/TOK")
	if !strings.Contains(got, "hello") {
		t.Fatalf("original body lost: %s", got)
	}
	if !strings.Contains(got, "--\nUnsubscribe: https://notify./v1/notify/unsubscribe/TOK") {
