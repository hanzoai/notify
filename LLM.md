...
brand/hanzo/plivo/{auth-id,…}      # fleet default (fail-closed if missing)
```
...
2. **Brand resolver** (`internal/tenant/plivo_resolver.go`) — used
   ...
   falls back to `brand/hanzo/plivo/*`. Fail-closed if the
   Hanzo default is missing — no hard-coded fallback. Surfaces
   via `/v1/notify/brand/plivo*` endpoints for the platform UI.

Why two paths: every Hanzo brand (hanzo, lux, zoo, pars, hanzo)
...
defaults to the Hanzo fleet account; brand admins promote to
